...
import { getStats, getOverdueSchedules, type StatsSnapshot } from "@/lib/supabase/admin"
...
export default async function StatsPage() {
  ...
  try {
    const [s, o] = await Promise.all([getStats(), getOverdueSchedules(20)])
    stats = s
    overdue = o
  } catch (e) {
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        <Kpi
          label="Nutzer (gesamt)"
          value={totalUsers}
          sub={`${activeUsers} aktiv · ${stats.users.banned} gesperrt`}
        />
        <Kpi
          label="Admins"
          value={stats.users.admins}
          sub="mit is_admin=true"
        />
        <Kpi
          label="Aktive Topics"
          value={stats.schedules.active}
          sub={`${stats.schedules.total} gesamt`}
        />
        <Kpi
          label="Reports (gesamt)"
          value={stats.reports.total}
          sub={`${stats.reports.last7d} in den letzten 7 Tagen`}
        />
      </div>
      ...
      <section className="bg-white rounded-xl border border-gray-100 shadow-sm p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
          ...
          <HealthCard
            ...
            sub={
              stats.schedules.overdueCount === 0
                ? "Alle Schedules pünktlich"
                : "next_run_at liegt > 6h in der Vergangenheit"
            }
          />
          <HealthCard
              ...
              {overdue.map(s => (
                <tr key={s.id} className="hover:bg-gray-50 transition-colors">
                  ...
                  <td className="px-4 py-3 text-xs">
                    {s.profile_email ? (
                      <Link
                        href={`/dashboard/users/${s.profile_id}`}
        ...
        return (
          <div key={plan} className="flex items-center gap-3 text-xs">
            ...
            <div className="flex-1 h-5 bg-[#f8f9fb] rounded-md overflow-hidden border border-gray-100">
              <div
                className="h-full bg-[#4F6EF7] transition-all"
                style={{ width: `${pct}%` }}
              />
            </div>
